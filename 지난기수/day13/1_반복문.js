...
for( let index = 0 ; index <= nameArray.length-1 ; index++ ){ // index는 0부터 마지막 인덱스 까지 1씩 증가
    ...
    for( let point = 1 ; point <= 100 ; point++  ){ // point 는 1부터 100까지 1씩 증가 반복 
        //console.log( point )
        if( point <= scores[index] ) { pointHTML += `●` } // 만약에 점수가 index번째 점수 이하이면 검은색o 대입 
        else{ pointHTML += `○` } // 내 점수보다 커지면 흰색o 대입 // = 대입 , +=누적대입
